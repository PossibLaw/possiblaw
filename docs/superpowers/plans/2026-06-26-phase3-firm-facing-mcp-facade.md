...
- **D1 — Where the facade lives → DECIDED: standalone `mcp-servers/firm-facade/` package** (mirrors `mcp-servers/legal-data/`). **Writer detail LOCKED via S6 (operator, 2026-06-26): import `gate-proxy/src/receipts.ts` for the `ReceiptBody`/`kind` TYPES only (extend the union with `"firm_facade"`); the facade does NOT instantiate a second `ReceiptChain` writer — it emits receipts by POSTing to a new gate-proxy HTTP endpoint so the gate proxy stays the single writer (one audit spine; `/receipts/verify` + `/receipts/bundle` cover facade actions).** State-changing facade actions fail-closed if the receipt can't be recorded; a facade-owned sibling ledger is the demo-only `--no-gate-proxy` fallback.
...
## SPIKE RESULTS — 2026-06-26 (RESOLVED)

All `file:line` against the pinned `paperclip/` submodule (read-only) unless noted. Routes are mounted under `/api`.

**S1 — matter (issue) create + status** ✅
- **Create:** `POST /api/companies/:companyId/issues` — `paperclip/server/src/routes/issues.ts:3143`. Validator `createIssueSchema` at `paperclip/packages/shared/src/validators/issue.ts:399`. Required: `title` (min 1). Optional: `description`, `projectId`, `parentId`, `status` (`backlog|todo|in_progress|in_review|done|blocked|cancelled`), `priority`, `assigneeAgentId`, `labelIds`, … Auth: `assertCompanyAccess` (`paperclip/server/src/routes/authz.ts:42`). Status auto-defaults to `todo` when an assignee is set, else `backlog`. Response = created issue.
- **Status read:** `GET /api/issues/:id` — `issues.ts:2142`, `assertCompanyAccess`. Returns `status` + `workProducts[]` + `documentSummaries[]` (metadata, **no body**) + `planDocument` (with body).

**S2 — work-product / document list + fetch** ✅ (drives D4)
- paperclip has TWO artifact entities: first-class **work products** (`issueWorkProducts`) and **documents** (key-keyed, carry `body` text). Work products are NOT comments and NOT documents — a separate entity.
- **List work products:** `GET /api/issues/:id/work-products` — `issues.ts:2407`, `assertCompanyAccess`. Metadata only (id, type [`preview_url|…|document`], provider, title, url?, status, reviewState, summary?, isPrimary…) — **no document body**.
- **List document metadata:** `GET /api/issues/:id` → `documentSummaries[]` (no body).
- **Fetch one document full text:** `GET /api/issues/:id/documents/:key` — `issues.ts:2433`, returns `body` (full text), `assertCompanyAccess`. (`GET /api/issues/:id/documents` — `issues.ts:2419` — returns ALL docs WITH body; use the per-key route to keep full-text disclosure scoped.)
- **Default-closed (D4) is clean:** `list_work_products` = work-products + documentSummaries (metadata only); `fetch_work_product` default = that metadata + dashboard link; full text (opt-in) = `GET /issues/:id/documents/:key`. **Task 3.4 build note:** map a work-product of `type:"document"` to its document `key`.

**S3 — approval create + deep link** ✅
- **Create:** `POST /api/companies/:companyId/approvals` — `paperclip/server/src/routes/approvals.ts:71`. Validator `createApprovalSchema` (`paperclip/packages/shared/src/validators/approval.ts:5`): `type` (required enum), `payload` (required record), `requestedByAgentId?`, `issueIds?`. Auth `assertCompanyAccess` (`authz.ts:42`). 201 → Approval record (`id`, `status`, …); payload redacted via `redactApprovalPayload()`.
- ⚠ **No generic "approve this work product" type.** Enum = `hire_agent | approve_ceo_strategy | budget_override_required | request_board_approval`. → `request_approval` (D2/D5) must map to **`request_board_approval`**, carrying human-readable context in `payload` + the matter in `issueIds`. **(Task 3.6 mapping — wants operator nod.)**
- **Decide:** `POST /api/approvals/:id/approve` (`approvals.ts:136`), `/reject` (`:232`), `/request-revision` (`:257`) — ALL `assertBoard` (`authz.ts:10`). Confirms D5.
- **Deep link:** `${PAPERCLIP_PUBLIC_URL}/:companyPrefix/approvals/:approvalId` — react-router `paperclip/ui/src/App.tsx:125`; `companyPrefix` = company `issuePrefix` (`App.tsx:159`); base-url env `PAPERCLIP_PUBLIC_URL` (`paperclip/server/src/config.ts:199`). Real, not invented.

**S4 — facade auth scope** ✅ (confirms risks #1, #5, #9)
- Create scope `assertCompanyAccess` (`authz.ts:42`) vs decide scope `assertBoard` (`authz.ts:10`, `req.actor.type==="board"` only).
- **No company-level key type exists** — only **agent API keys** (company-scoped) or board keys. Facade key = an **agent API key** (minted via `POST /api/agents/:id/keys`, `paperclip/server/src/routes/agents.ts:2850`, board-only to mint), sent `Authorization: Bearer <key>` — the gate-proxy precedent (`gate-proxy/src/paperclip-client.ts:43`, `PAPERCLIP_GATE_API_KEY`).
- ✅ **Invariant holds:** an agent-key actor CANNOT approve/reject/request-revision (`assertBoard` blocks it) — facade structurally cannot self-approve on an authenticated instance.
- ⚠ **local_trusted caveat:** unauthenticated loopback on a local_trusted instance is an implicit **board** actor (`paperclip/server/src/middleware/auth.ts:25`). So the structural can't-approve guarantee only holds on an AUTHENTICATED instance carrying an agent key; on local_trusted dev it rests on the facade simply not exposing an approve tool (same caveat the gate proxy documents). **→ known-limitations (Task 3.10).**

**S5 — MCP server template** ✅ LOCKED
- `mcp-servers/legal-data/src/server.ts` is the skeleton: `new McpServer({name,version})`; `server.registerTool(name, {description, inputSchema: {field: z.*}}, async handler)`; handler returns `{content:[{type:"text",text}]}` via `asContent()` (`server.ts:48`); `StdioServerTransport` + `await server.connect(transport)`. `inputSchema` is a **record of zod field validators** (not a `z.object`). Copy verbatim.

**S6 — receipt single-writer** ✅ RESOLVED — ⚠ refines D1
- `ReceiptChain` (`gate-proxy/src/receipts.ts:176`) is self-contained + restart-safe (derives seq/prevHash from the file's last line per append). `kind` union = `"egress"|"anchor"|"quality"` (`receipts.ts:24`).
- **Explicit single-writer / no-locking contract** (`receipts.ts:169-175,198`): "running two processes against the same file will corrupt the chain."
- The facade is **stdio (D3), spawned on demand by the outside assistant, and can run concurrently** with the gate-proxy HTTP server AND with other facade spawns. **→ directly importing `ReceiptChain` to append to the gate's shared file is unsafe** (second/third concurrent writer).
- **Resolution (recommended):** facade imports `receipts.ts` for the **`ReceiptBody`/`kind` types only** (extend the union with `"firm_facade"`) and emits receipts by **POSTing to a new gate-proxy HTTP endpoint**, so the gate proxy stays the **single writer** and the existing `/receipts/verify` + `/receipts/bundle` (Matter Trust Report) spine covers facade actions for free. State-changing facade actions (`create_matter`, full-text disclosure, `request_approval`) **fail-closed** if the receipt can't be recorded. Fallback (demo `--no-gate-proxy` only): a facade-owned sibling ledger, documented single-writer-assumed.
- ⚠ **Refines D1:** D1(a) "import the chain writer" → "import the *types*; write via the gate proxy's HTTP API (gate proxy = single writer)." Adds one small gate-proxy endpoint. **Wants operator nod** (one audit spine via HTTP vs a separate facade ledger).

### Three items to confirm before Task 3.1 — ✅ RESOLVED (operator, 2026-06-26)
1. **S6 / D1 writer detail → ROUTE THROUGH GATE PROXY.** Facade POSTs receipts to a new gate-proxy HTTP endpoint; gate proxy = single writer; one audit spine. (Adds Task: gate-proxy `POST /receipts/facade` endpoint + `"firm_facade"` kind, before/with Task 3.1.) Separate facade ledger is the `--no-gate-proxy` demo fallback only.
2. **S3 / Task 3.6 type mapping → ACCEPTED.** `request_approval` maps to `request_board_approval` (the only generic board-approval enum value); human-readable context in `payload`, matter linked via `issueIds`.
3. **S4 caveat → ACCEPTED.** Document the local_trusted "unauthenticated loopback = board" approval gap in known-limitations (Task 3.10).
