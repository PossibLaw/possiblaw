...
- Docs: `docs/roadmap.md` (buyer-facing scope + segment fit + honest limits), `docs/operator-walkthrough.md` (canonical getting-started), `docs/paperclip-package.md`, `docs/known-limitations.md`, `docs/ARCHITECTURE.md` (decision log).
...
- Egress trust proxy: `gate-proxy/` (standalone TypeScript; boundary classify → policy → anonymize → human gate → hash-chained receipts; citation gate enforced on court/third-party egress; RFC 3161 external anchoring via `GATE_TSA_URL` — fail-closed, tokens verify with `openssl ts -verify`). Tests: `pnpm -C gate-proxy test` (node:test).
