...
`pnpm -C trace-store test` - Run the trace-store node:test suite (64 tests: fail-closed config, canonical hashing, capture modes, role-gated visibility, per-matter store, retention purge, static lane resolution, plan evals).
