...
`python3 bin/_possiblaw_variants.py --self-test && python3 bin/_possiblaw_inline_source.py --self-test && python3 bin/_possiblaw_eval_coverage.py --self-test && python3 bin/_possiblaw_vendor_skill.py --self-test && python3 bin/_possiblaw_walls.py --self-test && python3 bin/_possiblaw_conflicts_screen.py --self-test` - Self-test the Python helpers.
...
`pnpm -C trace-store test` - Run the trace-store node:test suite (86 tests: fail-closed config, canonical hashing, capture modes, role-gated visibility, per-matter store, retention purge, static lane resolution, run poller + durable cursor, plan evals).
