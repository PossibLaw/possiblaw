---
name: pitch-polish-playbook
description: Authoritative checklist for tightening a pitch — clarity, value statement, evidence, call to action, and length.
---
# Pitch Polish Playbook

Follow these steps in order to transform a rough pitch draft into a crisp, compelling version.
...
5. **Verify the call to action (CTA)** — Every pitch must end with exactly one clear next step. Typical CTAs: "Schedule a 30-minute consultation at [LINK]," "Reply to request our capability statement," "Visit [URL] to learn more." If the draft has no CTA or has multiple competing CTAs, reduce to one.
...
8. **Verify length** — A pitch email should be 150–250 words. A pitch deck section (single slide narrative) should be 50–100 words. A one-pager executive summary should be 300–500 words. Trim to target range; do not pad to fill it.
...
10. **Label changes** — When returning the polished version, include a brief "Changes made" section listing 2–4 specific improvements and the rationale for each, so the operator can learn and accept or revert changes.
